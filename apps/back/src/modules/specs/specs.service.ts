...
export class SpecsService {
  ...
  async createSpecType(data: { key: string; label: string; description?: string }): Promise<any> {
    return prisma.specType.create({ data })
  }

  async updateSpecType(id: number, data: Partial<{ key: string; label: string; description: string }>): Promise<any> {

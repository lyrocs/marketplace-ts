...
export class DealsService {
    ...
    const [deals, total] = await Promise.all([
      prisma.deal.findMany({
        ...
        include: {
          ...
          products: {
            include: {
              product: { include: { category: true, brand: true, specs: { include: { spec: { include: { specType: true } } } } } },
            },
          },
        },
        orderBy: { createdAt: 'desc' },
  ...
  async update(
    ...
    await prisma.deal.update({
      ...
      data: {
        ...
        ...(dealData.description !== undefined && { description: dealData.description }),
        ...
        ...(dealData.invoiceAvailable !== undefined && { invoiceAvailable: dealData.invoiceAvailable }),
        ...(dealData.sellingReason !== undefined && { sellingReason: dealData.sellingReason }),
        ...(dealData.canBeDelivered !== undefined && { canBeDelivered: dealData.canBeDelivered }),
        ...(dealData.features !== undefined && { features: dealData.features }),
        ...(dealData.condition !== undefined && { condition: dealData.condition as any }),
      },
    })
    ...
    if (products !== undefined) {
      ...
      for (const product of products) {
        await prisma.dealProduct.create({
          data: { dealId: id, productId: product.productId, quantity: product.quantity },
        })
      }
    }
  ...
  async updateStatus(id: number, status: string, reason?: string): Promise<any> {
  ...
  async deleteImage(id: number, url: string): Promise<any> {
    const deal = await prisma.deal.findUnique({ where: { id } })
    if (!deal) throw new NotFoundException('Deal not found')
    const images = ((deal.images as string[]) || []).filter((img) => img !== url)
    return prisma.deal.update({ where: { id }, data: { images } })
  }

  async getPaginatedByStatus(status: string, page = 1, limit = 20): Promise<any> {

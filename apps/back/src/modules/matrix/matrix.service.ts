...
export class MatrixService {
  ...
  async start(): Promise<any> {
    ...
    this.client.on('Room.timeline' as any, async (event: any, room: any, toStartOfTimeline: boolean) => {
    ...
    try {
      const response = await fetch(
        `https://${matrixHost}/_synapse/admin/v2/users/${encodedUsername}`,
        {
          ...
          body: JSON.stringify({ password, admin: false, displayname: username }),
        },
      )
  ...
  async createRoom({ name, sellerName, buyerName }: { name: string; sellerName: string; buyerName: string }): Promise<string> {
    if (!this.client) return ''
